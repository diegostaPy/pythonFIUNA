...
for tam in tamanos:
    ...
    lista_aleatoria = [random.randint(1, tam) for _ in range(tam)]  # Lista aleatoria
    listas.append(("Ordenada", lista_ordenada))
    listas.append(("Inversa", lista_reversa))
    listas.append(("Aleatoria", lista_aleatoria))

# Comparar métodos de ordenamiento
for nombre, lista in listas:
    print(f"\nTipo de lista: {nombre}, Tamaño: {len(lista)}")
